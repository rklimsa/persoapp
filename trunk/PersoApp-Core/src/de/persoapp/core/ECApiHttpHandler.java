...
public final class ECApiHttpHandler implements HttpHandler {
	...
	public final void handle(final HttpExchange he) throws IOException {
		...
		// decode URL but replace resulting whitespace characters with "+", this happens in session IDs
		// from Governikus
		final URL tcTokenURL = qs != null && qs.contains(TC_TOKEN_URL) ? new URL(URLDecoder.decode(
				qs.substring(qs.indexOf(TC_TOKEN_URL) + TC_TOKEN_URL.length())).replace(' ', '+')) : null;
