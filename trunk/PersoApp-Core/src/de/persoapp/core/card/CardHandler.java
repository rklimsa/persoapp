...
public class CardHandler implements ICardHandler {
    ...
    public TransportProvider getECard() {
        if (this.tp0 != null) {
            final CardChannel cc = (CardChannel) this.tp0.getParent();
            if (cc != null) {
                try {
                    ...
                    // option #2: select MF, test pin state, but resets current
                    // card internal state
                    // this.tp0.transmit(new byte[] { 0x00, (byte) 0xA4, 0x00,
                    // 0x0C, 0x02, 0x3F, 0x00 });
                    // if (setMSE_AT(this.tp0, PACE_AES128CBC, (byte) 0x03,
                    // null) != -1) {
                    // return this.tp0;
                    // }
        ...
        if (tpNew != null) {
            ...
            // select ROOT / MF in case the card accepted selection of the
            // application instead of replying
            // with SW6982
            ...
            if (this.mainView != null) {
                if (manufacturer.length() > 0 && manufacturer.length() > 0) {
                    this.mainView.showMessage(this.textBundle.get("CardHandler_card_in_device") + manufacturer + " "
                            + product, IMainView.SUCCESS);
                } else {
                    this.mainView.showMessage(this.textBundle.get("CardHandler_device_not_verified")
                            + getCCID(tpNew).getName(), IMainView.WARNING);
                }
            }
    ...
    private int executeRemotePACE(final byte keyReference, final byte[] CHAT, final byte[] termDesc) {
        ...
        if (keyReference == 0x03 && setMSE_AT(this.tp, PACE_AES128CBC, keyReference, null) == 0x63C1) {
            pace_res = sendPACECommand(this.tp, 2, new byte[] { 2, 0, 0, 0, 0 }); // request
            // CAN
            // before
            // PIN
        }
        ...
        try {
            ...
            pace_res = sendPACECommand(this.tp, 2, data);
    ...
    private int executeLocalPACE(final String cryptoMechanism, final byte keyReference, final byte[] secret,
        ...
        status = setMSE_AT(this.tp, cryptoMechanism, keyReference, CHAT);
        if (keyReference == 0x03 && status == 0x63C1) {
            // request CAN before PACE
            final SecureHolder can = this.mainView.showCANDialog(this.textBundle.get("CardHandler_can_dialog_text"));
            status = executeLocalPACE(cryptoMechanism, (byte) 0x02, can.getValue(), null);
            if (status != 0x9000) {// Error by requesting the CAN.
                return status;
            }
            setMSE_AT(this.tp, cryptoMechanism, keyReference, CHAT);
        }
        ...
        try {
            ...
            final byte[] authData = generalAUTH(this.tp, null, false);
            if (this.tp.lastSW() == 0x9000 && authData != null) {
        ...
        final ECParameterSpec ecSpec = EC_Globals.getCurve(this.PACEv2_curveID);
        ...
        final byte[] paceQA = TLV.get(generalAUTH(this.tp, TLV.build(0x81, pace.init()), false), (byte) 0x82);

        /* retrieve public point relative to G' */
        /* send new public key to card */
        final byte[] paceYB = pace.step(paceQA);

        final byte[] paceYA = TLV.get(generalAUTH(this.tp, TLV.build(0x83, paceYB), false), (byte) 0x84);

        /* calculate common secret point between A and B */
        final byte[][] paceRes = pace.finish(paceYA);
        this.IDPICC = paceRes[0];
        ...
        try {
            ...
            final byte[] macResultA = generalAUTH(this.tp, TLV.build(0x85, cmac.doFinal()), true);
            status = this.tp.lastSW();
        ...
        if (status == 0x9000 && kEnc != null && kMac != null) {
            final ISOSMTransport smtp = new ISOSMTransport(this.tp);
    ...
    private int executePACE(final byte keyReference, final byte[] secret, final byte[] CHAT, final byte[] termDesc) {
        this.EFCardAccess = this.tp.transmit(new byte[] { 0x00, (byte) 0xB0, (byte) 0x9C, 0x00, 0x00, 0x00, 0x00 });
        ...
        // The chip card interface device - card terminal.
        final CCID ccid = getCCID(this.tp);
    ...
    public synchronized boolean startAuthentication(final byte CHAT[], final SecureHolder secret, final byte[] termDesc) {
        if (!this.initialized) {
            try {
                ...
                this.CAReferences = null;
                final byte secretType = 0x03;
                final int status = executePACE(secretType, secret != null ? secret.getValue() : null, CHAT, termDesc);

                if (status == 0x9000 && this.CAReferences != null) {
                    this.initialized = true;
                    log("CA-Reference: " + this.CAReferences);
                } else if (status == 0x6982) {
                    this.mainView.showError(this.textBundle.get("CardHandler_card_error_title"),
                            this.textBundle.get("CardHandler_card_error_text"));
                }
    ...
    public int doPINUnblock(final TransportProvider tp0, final byte verifySecret, final SecureHolder verifySecretInput,
        ...
        this.tp.transmit(new byte[] { 0x00, 0x2C, 0x03, unblockSecret });
        return this.tp.lastSW();
    }
    ...
    public int doPINChange(final TransportProvider tp0, final byte verifySecret, final SecureHolder verifySecretInput,
        ...
        status = -1;
        if (hasPACE(this.tp) > 0) {
            try {
                final byte[] res = getCCID(this.tp).modifyPinDirect(
            ...
            this.tp.transmit(buildCmd((byte) 0x00, (byte) 0x2C, (byte) 0x02, updateSecret,
                    updateSecretInput.getValue(), -1));
            status = this.tp.lastSW();
        }
    ...
    public boolean verifyCertificate(byte[] data) {
        ...
        this.lastCertSubject = TLV.get(data, (short) 0x5F20);
        data = TLV.build(0x83, TLV.get(data, (byte) 0x42));
        this.tp.transmit(buildCmd((byte) 0x00, (byte) 0x22, (byte) 0x81, (byte) 0xB6, data, -1));
        if (this.tp.lastSW() == 0x9000) {
            this.tp.transmit(buildCmd((byte) 0x00, (byte) 0x2A, (byte) 0x00, (byte) 0xBE, cert, -1));
            if (this.tp.lastSW() == 0x9000) {
                return true;
            }
        }
    ...
    public void initTA(final byte[] ephemeralKey, final byte[] auxData) {
        byte[] data = ArrayTool.arrayconcat(TLV.build((byte) 0x80, Hex.fromString("04007F00070202020203")),
                TLV.build((byte) 0x83, this.lastCertSubject));

        // check format of ephemeralKey if uncompressed indicator exists or not
        // and correct it
        // check if input is even -> indicator missing
        if (ephemeralKey.length % 2 == 0) {
            this.TAKey = ArrayTool.arrayconcat(new byte[] { 0x04 }, ephemeralKey);
        } else if (ephemeralKey[0] == 0x04) {
            this.TAKey = ephemeralKey;
        } else {
            // something went wrong
            throw new IllegalArgumentException("initTA ephemeralkey wrong: " + ephemeralKey.length + " / "
                    + ephemeralKey[0]);
        }

        // split x-coordinate away and include it, omit indicator byte
        data = ArrayTool.arrayconcat(data,
                TLV.build((byte) 0x91, ArrayTool.subArray(this.TAKey, 1, (this.TAKey.length - 1) / 2)));

        data = ArrayTool.arrayconcat(data, auxData);

        // tp.transmit("002281A400" + Hex.shortToString(data.length) +
        // Hex.toString(data));
        // transmit("002281A4", data);
        this.tp.transmit(buildCmd((byte) 0x00, (byte) 0x22, (byte) 0x81, (byte) 0xA4, data, -1));
    }
    ...
    public byte[] getTAChallenge() {
        return this.tp.transmit(new byte[] { 0x00, (byte) 0x84, 0x00, 0x00, 0x08 });
    }
    ...
    public boolean verifyTASignature(final byte[] signature) {
        this.tp.transmit(buildCmd((byte) 0x00, (byte) 0x82, (byte) 0x00, (byte) 0x00, signature, -1));
        if (0x9000 == this.tp.lastSW()) {
            return true;
        }
        return false;
    }
    ...
    public byte[] execCA() {
        ...
        this.tp.transmit(new byte[] { 0x00, 0x22, 0x41, (byte) 0xA4, 0x0C, (byte) 0x80, 0x0A, 0x04, 0x00, 0x7F, 0x00,
                0x07, 0x02, 0x02, 0x03, 0x02, 0x02 });
        return generalAUTH(this.tp, TLV.build(0x80, this.TAKey), true);
    }
    ...
    public byte[] transmit(byte[] cmd) {
        cmd = this.tp0.transmit(cmd);
        return ArrayTool.arrayconcat(cmd, new byte[] { (byte) (this.tp0.lastSW() >> 8),
                (byte) (this.tp0.lastSW() & 0xFF) });
    }
    ...
    public int doESignInit(final TransportProvider tp0) {
        ...
        if (status == 0x9000) {
            this.tp.transmit(new byte[] { (byte) 0x00, (byte) 0xE6, (byte) 0x10, ESIGN_PIN_ID });
            this.tp.transmit(new byte[] { (byte) 0x00, (byte) 0xE6, (byte) 0x21, (byte) 0x00, (byte) 0x05, (byte) 0xB6,
    ...
    public int doESignUnblock(final TransportProvider tp0) {
        ...
        if (status == 0x9000) {
            this.tp.transmit(new byte[] { (byte) 0x00, (byte) 0x2C, (byte) 0x03, ESIGN_PIN_ID });
        }
        return status;
    }
    ...
    public int validate_eSignPin(final TransportProvider tp0) {
        int status = -1;
        try {
            final byte[] res = getCCID(this.tp).verifyPinDirect(
    ...
    public byte[] doESign(final byte[] dataTBS) {
        return this.tp.transmit(buildCmd((byte) 0x00, (byte) 0x2A, (byte) 0x9E, (byte) 0x9A, dataTBS, 0));
    }
